...
- Native BoI Agent의 최종 답변은 Python typed tool loop가 만든 근거와 artifact를 기준으로 하고, `BOI_AGENT_COMPOSER_LLM_ENABLED=auto`일 때 OpenAI-compatible Gemma composer가 일반 구성원용 문장으로 다듬습니다. 운영 기본값은 `BOI_AGENT_COMPOSER_REQUIRED=1`이며, composer가 최종 답변을 만들지 못하면 deterministic 문장으로 숨기지 않고 `native_agent_runtime_unavailable` 장애로 표시합니다.
