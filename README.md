...
- NAS PoC의 Agent LLM 호출은 짧은 예산으로 제한합니다. 기본 `BOI_AGENT_STATUS_TIMEOUT_SECONDS=12`, `BOI_AGENT_COMPOSER_TIMEOUT_SECONDS=12`, `BOI_AGENT_COMPOSER_MAX_ATTEMPTS=1`이며, 작은 status/answer-plan JSON을 만들지 못하면 긴 대기나 규칙 기반 대체 답변 대신 Agent 장애로 노출합니다.
