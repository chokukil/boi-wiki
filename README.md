...
- `/api/agents/boi-wiki/chat`와 MCP `boi_agent_chat`: 현재 페이지와 ontology search, memory, inbox를 함께 써서 답변합니다. 내부적으로 Langflow `BoI Agent Flow` endpoint `boi-agent`를 호출하며, flow가 없으면 stub 답변 대신 `503 langflow_boi_agent_unavailable`을 반환합니다.
...
Langflow는 reasoning/orchestration backend입니다. 외부 agent와 사용자는 BoI API 또는 BoI Wiki MCP를 공식 인터페이스로 사용합니다.
