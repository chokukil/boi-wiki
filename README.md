...
- `/api/agents/boi-wiki/chat`와 MCP `boi_agent_chat`: 현재 페이지와 ontology search, memory, inbox를 함께 써서 답변합니다. `search/page_qa/summarize`는 fast path, `diagram/workflow_explain/gap_check/trace_reasoning`은 Langflow `BoI Agent Flow` endpoint `boi-agent` deep path로 보냅니다. flow가 없으면 stub 답변 대신 `503 langflow_boi_agent_unavailable`을 반환합니다.
