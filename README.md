...
- `/api/agents/boi-wiki/chat/stream`: Web Pet Agent용 SSE endpoint입니다. 오래 걸리는 질문은 `status` 한 줄 진행 상황을 먼저 보여주고, `answer_delta`로 답변을 누적한 뒤 `final` JSON으로 links/artifacts를 확정합니다.
