...
- Native BoI Agent의 최종 답변은 Python typed tool loop가 만든 근거와 artifact를 기준으로 하고, `BOI_AGENT_COMPOSER_LLM_ENABLED=auto`일 때 OpenAI-compatible Gemma composer가 일반 구성원용 문장으로 다듬습니다. 운영에서 composer를 필수로 보려면 `BOI_AGENT_COMPOSER_REQUIRED=1`을 설정하고, 실패 시 `native_agent_runtime_unavailable`으로 장애 처리합니다.
- `/api/agents/boi-wiki/chat/stream`: Web Pet Agent용 SSE endpoint입니다. 오래 걸리는 질문은 LLM stream planner가 만든 `route + status` 계획을 먼저 받고, `status` 한 줄 진행 상황을 보여주며, `answer_delta`로 답변을 누적한 뒤 `final` JSON으로 links/artifacts를 확정합니다. stream planner가 route/status JSON을 만들지 못하면 정해진 대체 문구를 쓰지 않고 `status_generation_failed` 또는 `boi_agent_router_unavailable` 장애로 표시합니다. `BOI_AGENT_STATUS_REQUIRED`는 호환용 설정명으로 남아 있지만 런타임 정책은 항상 필수이며, 값을 낮춰도 canned status fallback은 생기지 않습니다.
- `/api/agents/boi-wiki/suggestions`: 현재 페이지 추천 질문은 LLM suggestion writer가 만듭니다. `BOI_AGENT_SUGGESTIONS_REQUIRED=1` 운영 설정에서 writer가 실패하면 템플릿 질문으로 대체하지 않고 `boi_agent_suggestions_unavailable` 장애로 표시합니다.
