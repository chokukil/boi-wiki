...
| mutation/apply/promotion | user confirmation + RBAC + ACL + classification + audit. MCP `workflow_start`와 `promotion_submit`은 모든 실행/제출에서, `action_invoke`는 실제 실행(`dry_run=false`)에서 API 호출 전 `user_confirmed=true`를 요구한다. |
