...
| SOP 실행 상태 그래프 | `GET /api/sop-runs/{run_id}/graph` 또는 MCP `sop_run_graph` | 선택한 SOP 실행 인스턴스의 단계 상태, 근거 패킷, 판단 패킷 |
...
| private cleanup preview | `GET /api/private-memory/cleanup-preview` 또는 MCP `private_memory_cleanup_preview` | generated/background private BoI 정리 후보 확인 |
