...
- `deep`: Langflow `BoI Agent Flow`를 호출해 tool-loop reasoning 답변을 만든다.
...
- `diagram`, `workflow_explain`, `gap_check`, `trace_reasoning`: 항상 Langflow Deep Agent.
- `inbox`: Inbox API.
- `manual_complete`, `approval`: safety/confirmation flow.

예를 들어 “이 SOP를 Mermaid 프로세스 플로우로 보여줘”, “이 Event가 발생하면 어떤 Action이 이어져?”, “부족한 Action Spec 찾아줘”는 검색어가 포함되어 있어도 fast가 아니라 deep이다. Deep Agent 입력에는 `question`, `intent`, `current_url`, `page_context_pack`, `ontology_search_seed`, `employee_id`가 함께 들어가며, 최종 응답은 `answer_markdown`, `links`, `citations`, `suggested_questions`, `artifacts`, `context_summary` JSON contract를 따라야 한다. Mermaid는 `artifacts`와 Markdown code block 둘 다 제공한다.
