...
NAS Git Auto-Pull은 `/volume1/docker/boi-wiki/app` Git worktree가 `main` 브랜치를 주기적으로 따라오게 하는 운영 자동화다. DSM Scheduled Task가 `scripts/nas_auto_pull_deploy.sh`를 실행하고, 스크립트는 fast-forward 가능한 변경만 pull한다.
...
DSM Task Scheduler에서 root 권한 task를 만들고 1~5분 주기로 다음 명령을 실행한다.

```bash
mkdir -p /volume1/docker/boi-wiki/deploy-logs
cd /volume1/docker/boi-wiki/app && \
  /usr/bin/env bash scripts/nas_auto_pull_deploy.sh \
  >> /volume1/docker/boi-wiki/deploy-logs/autopull.log 2>&1
```

로그의 마지막 줄은 항상 다음 중 하나여야 한다.
