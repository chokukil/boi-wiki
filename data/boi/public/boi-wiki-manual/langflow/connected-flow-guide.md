...
`BoI Agent Flow`는 Web Pet Agent와 MCP `boi_agent_chat`의 trusted deep-reasoning backend다. 사용자와 외부 agent는 Langflow URL을 직접 호출하지 않고 BoI API 또는 `boi-wiki-mcp`를 호출한다. BoI API는 LLM Router로 `route`와 `intent`를 먼저 분류하고, 단순 검색/요약은 Page Context fast path로 답한다. `diagram`, `workflow_explain`, `gap_check`, `trace_reasoning` intent는 Langflow `BoI Agent Flow`로 전달한다. Langflow canvas 안에서는 native Agent component가 Gemma model을 reasoning engine으로 쓰고, `BoI Agent Tools` custom component의 `ontology_search`, `boi_get`, `action_spec_lookup`, `workflow_status`, `agent_inbox`, `manual_handoff_complete`, `memory_recall` tool을 반복 호출한다. `boi_answer`는 fallback compact-answer tool이며 정상 deep reasoning의 첫 tool로 고정하지 않는다. recursion 방지를 위해 `boi_agent_chat` tool은 Agent toolset에 넣지 않는다.
