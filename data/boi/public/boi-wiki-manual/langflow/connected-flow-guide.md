...
| Agentic simulator | Input -> ContextNormalizer -> BoISimulationAgent -> BoIPromptComposer -> Gemma LLM -> BoIWikiWriter/BoIActionInvoker -> BoIResultComposer -> ChatOutput |
...
Universal Simulator는 `BoI Simulation Agent`를 중심으로 둔다. Agent는 Action Catalog, Event Type, SOP stage, 같은 trace의 event/action log, generated BoI, prior action result를 seed로 삼아 BoI Wiki를 bounded loop로 여러 번 조회하고, coverage report와 retrieval trace를 만든다. Gemma LLM은 이 agent context를 받아 최종 표현과 요약을 담당한다. 따라서 공식 simulator 예제는 `BoI Simulation Agent` 노드가 없거나 prompt path에 연결되지 않으면 완료로 보지 않는다.

Action Gateway가 이미 `simulation_agent` context를 만든 경우 Langflow component는 같은 API를 다시 호출하지 않고 pass-through한다. Langflow UI에서 직접 실행하는 경우에만 component가 `/api/simulations/universal-agent`를 호출한다. Universal flow 안에 `manual.direct_development.decide_cross_section` 같은 stale hardcoded action key가 남아 있으면 runtime audit 실패로 본다.
...
- Universal Simulator contains `BoI Simulation Agent`, and its output is connected to the prompt path.
