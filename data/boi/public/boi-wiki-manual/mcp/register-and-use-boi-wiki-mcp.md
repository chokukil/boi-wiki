...
Codex에서 BoI Wiki MCP를 사용할 때 이름은 `boi-wiki-mcp`, transport는 Streamable HTTP, URL은 `http://localhost:8200/mcp`로 둔다. 등록 후 `boi_search`, `boi_get`, `workflow_status`, `action_invoke`, `event_type_draft_create`, `capabilities_search` 같은 tool이 보이면 정상이다.

BoI Agent 관련 tool은 BoI API와 같은 guardrail을 사용한다. `boi_search`는 document-only search이고, 복합 업무 탐색은 `ontology_search`, 현재 페이지 기반 질의응답은 `boi_agent_chat`, 담당 업무 확인은 `agent_inbox`를 사용한다. `capabilities_search`, `capability_get`, `capability_deduplicate`, `event_skills_list`, `action_skills_list`는 API/MCP/Webhook/Langflow/Manual/Skill/SOP 등록 전 기존 Capability와 Skill을 찾고 중복 생성을 피하는 도구다. `boi_agent_capabilities`는 Native Agent backend, 응답 계약, streaming event, guardrail, 지원 execution card를 `/api/agents/boi-wiki/capabilities`와 같은 형태로 반환한다. `boi_agent_approve`는 `execution_cards[]`의 `operation`과 `payload`를 사용해 Web Pet Agent와 같은 `/api/agents/boi-wiki/approve` 승인 경로를 호출한다. `rbac_me`, `rbac_check`, `doc_access_check`, `rbac_audit`는 외부 Agent가 실행 전 권한과 문서 접근 가능성을 같은 정책으로 확인하고, 실행 후 audit trail을 점검하기 위한 read/check tool이다. `boi_agent_chat`은 Web Pet Agent와 같은 입력 필드(`question`, `mode`, `intent`, `current_url`, `selected_text`, `page_context`, `conversation`, `save_memory`)를 BoI API로 전달하고, 같은 `boi-agent.response.v1` 응답 계약을 반환한다. 응답에는 `answer_markdown`, `display_markdown`, `links`, `citations`, `artifacts`, `execution_cards`, `status_updates`, `status_events`, `tool_trace`, `evidence_ledger`, `affordances`, `answer_quality`, `access_summary`, `guardrails_applied`, `suggested_questions`, `event_context`, `capability_context`가 포함된다. `status_updates`가 canonical field이고 `status_events`는 streaming event 모델을 쓰는 외부 client를 위한 같은 값의 alias다. `suggested_questions`는 page starter가 아니라 방금 답변의 evidence ledger, artifact, affordance를 바탕으로 만든 answer-scoped follow-up이다. Web Pet의 streaming 상태 문구는 MCP에서 SSE로 오지 않고 최종 JSON의 `status_updates`와 `status_events`에 들어간다. MCP client는 `answer_html`을 필수로 해석하지 말고 `display_markdown`과 typed `artifacts`/`execution_cards`를 자기 UI에 맞게 렌더링한다. REST client는 `/api/agents/boi-wiki/response-schema`, MCP client는 `boi://agent/response-schema/latest` resource template으로 같은 JSON Schema를 읽어 응답을 검증할 수 있다. 이 JSON Schema의 canonical source는 BoI API이며, MCP resource template은 MCP 내부 상수 복제본이 아니라 BoI API schema endpoint를 읽어 반환한다. 신규 Event Type은 `event_type_draft_create`로 private draft BoI와 catalog patch proposal을 만들고 `event_type_draft_validate`로 검증한 뒤, 별도 승인 후 `event_type_draft_apply`로 catalog에 반영한다. Apply가 완료되면 같은 private draft BoI는 적용 기록과 catalog entry, source edit 결과를 담은 `reviewed` 문서로 동기화된다. `boi_agent_approve`, `manual_handoff_complete`, `event_type_draft_create`, `event_type_draft_apply`, apply, promotion, action 실행 계열은 사용자 확인과 RBAC/ACL 검증 없이는 실행되지 않는다.
...
| `capabilities_search` / `capability_get` / `capability_deduplicate` | Capability Pack 조회, 상세 확인, 신규 등록 전 중복 후보 산출 |
...
상태 페이지는 서버 health뿐 아니라 실제 MCP capabilities 목록과 MCP auth 상태를 보여준다. `tools=37`, `resource_templates=11`, `prompts=5`, `resources=0`이 현재 기준이며, `resources=0`은 정적 resource 대신 resource template을 쓰는 설계라서 정상이다. 상태 페이지의 목록에는 `boi_agent_chat`, `boi_agent_capabilities`, `boi_agent_approve`, `ontology_search`, `agent_inbox`, `manual_handoff_complete`, `rbac_me`, `rbac_check`, `doc_access_check`, `rbac_audit`, Event Type draft tool, Capability/Skill registry tool, `boi://employees/{employee_id}/...` resource template, `boi://agent/response-schema/{version}`가 함께 보여야 한다. 외부에 공개된 endpoint에서는 `mcp_auth.required=true`가 권장된다.
