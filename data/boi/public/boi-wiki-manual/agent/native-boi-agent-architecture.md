...
sequenceDiagram
  ...
  API->>API: LLM status writer creates status plan
  API-->>UI: status generated by LLM
...
`status` event는 사용자가 장시간 요청을 멈춘 것으로 오해하지 않도록 한 줄 진행 상황만 전달한다. 이 문구는 code rule이 아니라 OpenAI-compatible Gemma status writer가 생성한다. status writer가 실패하면 정해진 문구로 대체하지 않고 `error` event의 `status_generation_failed`로 중단한다. 실제 최종 응답의 canonical contract는 `final` event의 JSON이며, 기존 `/chat` 응답과 같은 `answer_markdown`, `answer_html`, `links`, `citations`, `artifacts`, `context_summary`, `route`, `intent` 필드를 유지한다.
