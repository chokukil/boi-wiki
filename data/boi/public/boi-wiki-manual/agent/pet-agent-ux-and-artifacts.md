...
| Event | UI behavior |
|---|---|
| `status` | 말풍선 위 작은 진행 줄에 마지막 상태 한 줄을 표시한다. 예: `현재 화면 맥락을 확인하고 있습니다.` |
...
기본 화면에서 사용자가 계속 보게 되는 진행 정보는 `status`의 마지막 한 줄이다. 상세 진행 이력은 같은 말풍선 안의 접힌 `진행 단계` details에 보관해 디버깅과 설명 가능성은 유지하되, 긴 Agent 요청 중 화면을 진행 로그로 채우지 않는다.
