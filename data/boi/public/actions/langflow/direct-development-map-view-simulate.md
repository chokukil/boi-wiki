...
description: Map 분석 시스템의 Map View Image 확인을 BoI Universal Action Simulator Flow로 시뮬레이션한다.
tags:
- DirectDevelopment
- ActionGateway
- langflow
timestamp: '2026-06-21T09:10:00+09:00'
boi_id: boi:public:actions:langflow:direct-development-map-view-simulate
visibility: public
...
request_schema:
  ...
  properties:
    action_key:
      const: direct_development.map_view.simulate
    employee_id:
      type: string
    event:
      type: object
...
example_response:
  ok: true
  status: langflow_invoked
  action: direct_development.map_view.simulate
  simulation: true
  simulation_label: SIMULATED
  result:
...
`direct_development.map_view.simulate`는 [직개발 결과 확인 및 Reporting SOP](/public/sop/direct-development-reporting.md)의 실행 단계에 연결된 action spec이다.
