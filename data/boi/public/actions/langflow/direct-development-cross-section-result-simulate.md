...
title: 단면검사 결과 확인 시뮬레이션
description: 단면 검사 시스템 결과 확인을 BoI Universal Action Simulator Flow로 시뮬레이션한다.
tags:
- DirectDevelopment
- ActionGateway
- langflow
timestamp: '2026-06-21T09:10:00+09:00'
boi_id: boi:public:actions:langflow:direct-development-cross-section-result-simulate
visibility: public
...
status: reviewed
action_key: direct_development.cross_section_result.simulate
...
request_schema:
  ...
  properties:
    action_key:
      const: direct_development.cross_section_result.simulate
    employee_id:
      type: string
    event:
      type: object
...
example_response:
  ok: true
  status: langflow_invoked
  action: direct_development.cross_section_result.simulate
  simulation: true
  simulation_label: SIMULATED
  result:
...
curl: 'curl -X POST ''http://localhost:8100/api/actions/invoke'' -H ''x-service-token:
  $SERVICE_TOKEN'' -H ''Content-Type: application/json'' -d ''{"action_key":"direct_development.cross_section_result.simulate","employee_id":"100001","event":{"event_type":"direct_development.cross_section.requested.v1","trace_id":"trace-direct-development-demo"},"payload":{"tech":"Tech-A","work_id":"1.10","owner":"100001"}}'''
action_gateway_mapping:
  invoke_url: http://localhost:8100/api/actions/invoke
  action_key: direct_development.cross_section_result.simulate
  catalog_type: langflow_run
  doc_ref: boi:public:actions:langflow:direct-development-cross-section-result-simulate
...
`direct_development.cross_section_result.simulate`는 [직개발 결과 확인 및 Reporting SOP](/public/sop/direct-development-reporting.md)의 실행 단계에 연결된 action spec이다.
