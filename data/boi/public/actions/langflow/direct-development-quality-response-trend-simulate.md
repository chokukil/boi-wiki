...
description: 품질 시스템 Response Trend 확인을 BoI Universal Action Simulator Flow로 시뮬레이션한다.
  실제 품질 시스템 호출이 아니다.
tags:
- DirectDevelopment
- ActionGateway
...
request_schema:
  ...
  properties:
    action_key:
      const: direct_development.quality_response_trend.simulate
    employee_id:
      type: string
    event:
      type: object
...
example_response:
  ok: true
  status: langflow_invoked
  action: direct_development.quality_response_trend.simulate
  simulation: true
  simulation_label: SIMULATED
  result:
...
curl: 'curl -X POST ''http://localhost:8100/api/actions/invoke'' -H ''x-service-token:
  $SERVICE_TOKEN'' -H ''Content-Type: application/json'' -d ''{"action_key":"direct_development.quality_response_trend.simulate","employee_id":"100001","event":{"event_type":"direct_development.result_check.requested.v1","trace_id":"trace-direct-development-demo"},"payload":{"tech":"Tech-A","work_id":"1.10","owner":"100001"}}'''
...
`direct_development.quality_response_trend.simulate`는 [직개발 결과 확인 및 Reporting SOP](/public/sop/direct-development-reporting.md)의 실행 단계에 연결된 action spec이다.
