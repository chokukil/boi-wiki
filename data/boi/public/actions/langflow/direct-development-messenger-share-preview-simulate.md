...
title: 협의체 공유 Preview 시뮬레이션
description: 메신저 공유 전 preview를 BoI Universal Action Simulator Flow로 생성한다. 실제 메신저 발송이
  아니다.
tags:
- DirectDevelopment
- ActionGateway
...
boi_id: boi:public:actions:langflow:direct-development-messenger-share-preview-simulate
visibility: public
...
status: reviewed
action_key: direct_development.messenger_share_preview.simulate
...
request_schema:
  ...
  properties:
    action_key:
      const: direct_development.messenger_share_preview.simulate
    employee_id:
      type: string
    event:
      type: object
...
example_response:
  ...
  simulation_label: SIMULATED
  result:
    message: 'SIMULATED: 실제 메신저 발송이 아니라 BoI Universal Action Simulator Flow가 생성한 PoC
      preview입니다.'
curl: 'curl -X POST ''http://localhost:8100/api/actions/invoke'' -H ''x-service-token:
  $SERVICE_TOKEN'' -H ''Content-Type: application/json'' -d ''{"action_key":"direct_development.messenger_share_preview.simulate","employee_id":"100001","event":{"event_type":"direct_development.share.requested.v1","trace_id":"trace-direct-development-demo"},"payload":{"tech":"Tech-A","work_id":"1.10","owner":"100001"}}'''
action_gateway_mapping:
  invoke_url: http://localhost:8100/api/actions/invoke
  action_key: direct_development.messenger_share_preview.simulate
  catalog_type: langflow_run
  doc_ref: boi:public:actions:langflow:direct-development-messenger-share-preview-simulate
...
`direct_development.messenger_share_preview.simulate`는 [직개발 결과 확인 및 Reporting SOP](/public/sop/direct-development-reporting.md)의 실행 단계에 연결된 action spec이다.
