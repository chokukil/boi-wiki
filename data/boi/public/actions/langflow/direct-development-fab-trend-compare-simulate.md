...
title: 연구소-양산 FAB 비교 Trend 확인 시뮬레이션
description: 품질 시스템의 연구소-양산 FAB 비교 Trend 확인을 BoI Universal Action Simulator Flow로
  시뮬레이션한다.
tags:
- DirectDevelopment
- ActionGateway
...
request_schema:
  ...
  properties:
    action_key:
      const: direct_development.fab_trend_compare.simulate
    employee_id:
      type: string
    event:
      type: object
...
example_response:
  ok: true
  status: langflow_invoked
  action: direct_development.fab_trend_compare.simulate
  simulation: true
  simulation_label: SIMULATED
  result:
    message: 'SIMULATED: 실제 품질 시스템 호출이 아니라 BoI Universal Action Simulator Flow가 생성한
      PoC 결과입니다.'
curl: 'curl -X POST ''http://localhost:8100/api/actions/invoke'' -H ''x-service-token:
  $SERVICE_TOKEN'' -H ''Content-Type: application/json'' -d ''{"action_key":"direct_development.fab_trend_compare.simulate","employee_id":"100001","event":{"event_type":"direct_development.fab_trend.compare_requested.v1","trace_id":"trace-direct-development-demo"},"payload":{"tech":"Tech-A","work_id":"1.10","owner":"100001"}}'''
action_gateway_mapping:
  invoke_url: http://localhost:8100/api/actions/invoke
  action_key: direct_development.fab_trend_compare.simulate
  catalog_type: langflow_run
  doc_ref: boi:public:actions:langflow:direct-development-fab-trend-compare-simulate
...
`direct_development.fab_trend_compare.simulate`는 [직개발 결과 확인 및 Reporting SOP](/public/sop/direct-development-reporting.md)의 실행 단계에 연결된 action spec이다.
