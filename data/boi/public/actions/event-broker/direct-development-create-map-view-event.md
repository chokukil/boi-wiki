...
title: Map View 확인 이벤트 발행
description: Response Trend 확인 후 Map View 확인 stage를 시작하는 이벤트를 발행한다.
tags:
- DirectDevelopment
- ActionGateway
- event_broker
timestamp: '2026-06-21T09:10:00+09:00'
boi_id: boi:public:actions:event-broker:direct-development-create-map-view-event
visibility: public
...
status: reviewed
action_key: direct_development.create_map_view_event
...
request_schema:
  ...
  properties:
    action_key:
      const: direct_development.create_map_view_event
    employee_id:
      type: string
    event:
      type: object
...
example_response:
  ok: true
  status: event_published
  action: direct_development.create_map_view_event
  simulation: false
...
curl: 'curl -X POST ''http://localhost:8100/api/actions/invoke'' -H ''x-service-token:
  $SERVICE_TOKEN'' -H ''Content-Type: application/json'' -d ''{"action_key":"direct_development.create_map_view_event","employee_id":"100001","event":{"event_type":"direct_development.result_check.requested.v1","trace_id":"trace-direct-development-demo"},"payload":{"tech":"Tech-A","work_id":"1.10","owner":"100001"}}'''
action_gateway_mapping:
  invoke_url: http://localhost:8100/api/actions/invoke
  action_key: direct_development.create_map_view_event
  catalog_type: event_publish
  doc_ref: boi:public:actions:event-broker:direct-development-create-map-view-event
  flow_name: null
  resolve_latest: null
health_check:
  type: http
...
`direct_development.create_map_view_event`는 [직개발 결과 확인 및 Reporting SOP](/public/sop/direct-development-reporting.md)의 실행 단계에 연결된 action spec이다.
